...
def conv1d_fft(x, weights, dim = -2, weight_dim = -1):
    ...
    out = out.roll(-1, dims = (dim,)) # roll to match torch.nn.functional.conv1d

    indices = torch.arange(start = fast_len - N, end = fast_len, dtype = torch.long, device = x.device) # get the last N elements
...
class SingleHeadedAttention(nn.Module):
    ...
    def forward(self, x, v_input = None):
        ...
        v_input = default(v_input, x) # if v_input is None, use x
...
# 
    ...
    def forward(self, x):
        ...
        def apply_learned_ema_with_damping(x, alphas, dampen_factors):
            ...
            K = alphas * (((1 - alphas) * dampen_factors) ** rearrange(reversed_powers, '... l -> ... l 1')) # eq26
...
if __name__ == '__main__':
    ...
    x2 = torch.randint(0, 256, (1, 1024))     # (batch, seq, dim)

    out = model(x2) # (1, 1024, 128)    
    print(out.shape)